...
static FuncReturn_t process_tx_msg (void){
  ...
  if(is_action_completed_fail())
  {
    clear_complete_fail_evnt();
    ...
    case MSG_RX_RQST:
      {
        ...
        _set_sftclk_offset(maMessageTO[current_msg]);
        command_xcvr_rx();
...
void process_scan_req(void)
{
  ...
  static channel_num_t bestChann = CHANNEL11;
  ...
  if(MSG_ED_ACTION_COMPLETE_SUCCESS == Scan_msg.u8Status.msg_state)
  {
    ...
    if( (0 != (u8ScanValPerChann[EdCh2Evaluate])) ||(TRUE == isSecondPass))
    {
