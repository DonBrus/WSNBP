...
#include "../Configure/options_config.h"
#include "../Drivers/Configure/board_config.h"
...
*/
#define MAJOR_SMAC_V  (1)
#define MIDDLE_SMAC_V (1)
#define MINOR_SMAC_V  (3)
#define YEAR_SMAC_V   (2009-2000)
#define MONTH_SMAC_V  (8)
#define DAY_SMAC_V    (4)
#define CONS_SMAC_V   (1)

#if (MAJOR_SMAC_V>15) || (MIDDLE_SMAC_V>31) || (MINOR_SMAC_V>31)
 #error "Cobdebase version out of range"
#endif

#if (YEAR_SMAC_V>31) || (MONTH_SMAC_V>12) || (MONTH_SMAC_V<1) || (DAY_SMAC_V>31) || (DAY_SMAC_V<1)
...
const uint32_t smac_version = ((MAJOR_SMAC_V << 28) | (MIDDLE_SMAC_V << 23 ) | \
                               (MINOR_SMAC_V << 18) | (YEAR_SMAC_V << 13 ) | \
                               (MONTH_SMAC_V << 9) | (DAY_SMAC_V << 4 ) | CONS_SMAC_V);


static bool_t mIsPromiscousMode = FALSE;
static const version_and_size_tag version_registers[MAX_VERSIONED_ENTITY] = {
  {(uint32_t *)(&maca_mc1322x_id), 1},     /* MACA_MC1322x_ID VERSION */
...
FuncReturn_t MLMERadioInit(void)
{
  ...
  SetComplementaryPAState(gEnableComplementaryPAOutput_c);
  SetPowerLevelLockMode(gPowerLevelLock_c);
  SetEdCcaThreshold(gCcaThreshold_c);
...
static void InitializePhy(void)
{
  ...
  MACA_WRITE(maca_tmren      , maca_start_clk | maca_soft_clk);          // 0x80004040
  ...
  MACA_WRITE(maca_maskirq    , (maca_irq_cm   | maca_irq_acpl | 
                                maca_irq_rst  | maca_irq_di | maca_irq_crc | maca_irq_flt ));
